// File: CVtoMIDI_V8p6_MicroGlissOnly.ino
// 0–5V (1V/oct). Двухточечная калибровка (D9), анти-boot-note, фикс «форшлага» на LGT.
// Новое: без «длинного глиссандо» — прыжок при |Δ|≥2; микро-шаг только на соседний полутон.
// Плюс комбинированный «без глиссандо» (#define и/или пин D10).
...
const uint16_t CAL_HOLD_MS      = 1500;

// «Без глиссандо»: #define и/или D10
...
// Ограничить глисс до микро-шага (±1); при |Δ|≥2 всегда jump
const bool MICRO_GLISS_ONLY = true;
...
// LGT-фикс: стабилизация питча перед NoteOn
const uint16_t NOTEON_DELAY_MS          = 12;
const uint16_t NOTEON_STAB_WINDOW_MS    = 30;
const uint8_t  NOTEON_STABLE_COUNT      = 2;
...
CvSample readAdcTrimmed(uint8_t n,uint8_t keep,uint16_t dur_ms){
  ...
  uint8_t keepn=(uint8_t)((uint16_t)n*keep/100); if(keepn<1)keepn=1; if(keepn>n)keepn=n;
...
// CAL button
inline bool calRawActive(){ int r = digitalRead(CAL_PIN); return CAL_ACTIVE_LOW ? (r==LOW) : (r==HIGH); }
bool tryEnterCalibrationByButton(){
  static bool armed = true, prev=false; static unsigned long tDown = 0;
...
// NO-GLISS
inline bool noGlissPinActive(){ int r = digitalRead(NO_GLISS_PIN); return NO_GLISS_ACTIVE_LOW ? (r==LOW) : (r==HIGH); }
...
bool calibrationWizard(){
  ...
  ledBlink(3,250,250); in_calibration = false; return true;
}

// Quantize & stable NoteOn
inline int quantizeFromAdc(float adc_avg){
  float semi_abs = (float)ZERO_VOLT_MIDI + g_a * (adc_avg - g_adc_zero);
  return clampMidi(fastRoundToInt(semi_abs));
}
int measureNoteStable(uint16_t window_ms){
  const unsigned long t0 = millis(); int prev = -999; uint8_t same = 0; int last = -999;
  while(millis() - t0 < window_ms && gate_active_stable){
    (void)analogRead(PIN_CV); (void)analogRead(PIN_CV); // WHY: LGT мультиплексор
    int n = quantizeFromAdc(readAdcOversampled().adc_avg);
    last = n;
    if(n == prev){ if(++same >= NOTEON_STABLE_COUNT) return n; }
    else { prev = n; same = 1; }
    delay(2);
  }
  return (prev!=-999) ? prev : ((last!=-999)? last : 60);
}
...
void loop(){
  ...
  // NoteOn — стабилизировано
  if(REQUIRE_EDGE_FOR_FIRST){
    if(note_armed && !gate_active_prev && gate_active_stable){
      delay(NOTEON_DELAY_MS);
      if(!gate_active_stable) goto after_edges;
      current_note = measureNoteStable(NOTEON_STAB_WINDOW_MS);
...
after_edges:
  ...
  if(gate_active_stable && note_on && LEGATO_TRACKING){
    ...
    if(now - last_scan_ms >= 5){
      last_scan_ms=now;
      float adc = readAdcOversampled().adc_avg;
      float semi_abs = (float)ZERO_VOLT_MIDI + g_a * (adc - g_adc_zero);
      int q = clampMidi(fastRoundToInt(semi_abs));
      int diff = q - current_note;
      ...
      if(noGlissEffective()){
        ...
        if(semi_abs >= up_thr || semi_abs <= down_thr){
          if(MICRO_GLISS_ONLY && abs(diff) >= 2){
            // прыжок при больших скачках
            midiNoteOff(current_note);
            current_note = q;
            updateSchmittThresholds(current_note);
            midiNoteOn(current_note, MIDI_VELOCITY);
            ...
            // микро-шаг (±1)
            int next = current_note + (diff>0 ? 1 : (diff<0 ? -1 : 0));
